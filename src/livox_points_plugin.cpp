...
void LivoxPointsPlugin::Load(gazebo::sensors::SensorPtr _parent, sdf::ElementPtr sdf) {
    ...
    SendRosTf(sensor_pose, "base_link", raySensor->Name());
    // ROS_ERROR_STREAM("raySensor->Name()"<< raySensor->Name());
    // ROS_ERROR_STREAM("raySensor->ParentName()"<< raySensor->ParentName());
...
void LivoxPointsPlugin::PublishPointCloud(std::vector<std::pair<int, AviaRotateInfo>> &points_pair) {
    ...
    // ROS_ERROR_STREAM("pub successful");
    // ROS_ERROR_STREAM("scan_point.points.size()"<<scan_point.points.size());
    ros::spinOnce();
